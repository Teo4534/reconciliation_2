...
# ---------------------------------------------------------------- fee rules (mirror of Rates)
RATE = {1: 19.50, 2: 34.50, 3: 46.00, 4: 57.10}      # per family per session, by sibling count
...
for i in range(n_families):
    # three families carry a double-barrelled name whose two halves are *also* separate families —
    # this is the ambiguity that breaks naive surname matching, and it exists in the real data
    if i < len(DOUBLE):
        sur = "-".join(DOUBLE[i])
    else:
    ...
    for _ in range(n_kids):
        ...
        used_first.add(f); kids.append(f)
    families.append(dict(sur=sur, kids=kids, parent=rng.choice(PARENT_FIRST)))

# invoice numbers, with one number deliberately issued to two families
inv_numbers = {}
n = 1
for i, fam in enumerate(families):
    inv_numbers[i] = f"{TERM_CUR}-{n:03d}"; n += 1
shared_a, shared_b = 7, 34
inv_numbers[shared_b] = inv_numbers[shared_a]          # the shared-invoice defect
...
# a few defects that exist in the real roster
roster_rows[3]["OFFICE SUPPLIES"] = 18.0                              # wrong supplies charge
roster_rows[11]["OFFICE SUPPLIES"] = -30.0                            # discount stored as negative supplies
roster_rows[19]["INVOICE NUMBER"] = ""                                # enrolled, never invoiced
...
# ---------------------------------------------------------------- bank export
def memo(payer, ref):
    """the bank writes a fixed-width memo: 23 chars of payer, then a 21-char reference slot"""
    return f"{payer[:23]:<23}\t{ref[:21]}\tBGC\t"

def expected_total(i):
    k = len(families[i]["kids"])
...
payers = [i for i in range(n_families) if expected_total(i) > 0]     # the un-invoiced family never pays
for idx, i in enumerate(rng.sample(payers, min(len(modes), len(payers)))):
    ...
    if mode == "clean":            ref = ref_clean
    elif mode == "glued":          ref = f"{fam['sur'][:6].upper()}{ref_clean}"
    elif mode == "no_hyphen":      ref = rng.choice([ref_clean.replace("-", ""), ref_clean.replace("-", " - ")])
    elif mode == "prior_term":     ref = f"{TERM_PRIOR}-{rng.randint(500, 660):03d}"
    elif mode == "bare":           ref = ref_clean.split("-")[1]
    elif mode == "name_only":      ref = rng.choice([fam["sur"], f"{fam['sur']} fees", "school fees"])
    elif mode == "first_name_only": ref = rng.choice(fam["kids"])
    elif mode == "wrong_ref":      ref = inv_numbers[(i + 1) % n_families]
    elif mode == "truncated":      ref = f"{fam['sur'].upper()} FRENCH SCHOOL {ref_clean}"[:21]
    elif mode == "shared_invoice": ref = inv_numbers[shared_a]; i = rng.choice([shared_a, shared_b])
    ...
    if part:                                            # the second instalment, often with a worse reference
...
print("failure modes:", {m: sum(1 for t in truth if t['failure_mode'].startswith(m)) for m, _ in FAILURE_MODES})
